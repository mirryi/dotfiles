...
var d = document,
  ...
  trusted = ["greasyfork.org", "w9p.co"],
  imgtab =
    d.images.length == 1 && d.images[0].parentNode == d.body && !d.links.length,
  cfg = loadCfg(),
  enabled = cfg.imgtab || !imgtab,
  _ = {},
  hosts;
...
function fixCfg(s, save) {
  ...
  else if (cfg.version == def.version) return cfg;
  for (var dp in def) {
...
function loadHosts() {
  var hosts = [
    ...
    { r: /[\/\?=](https?.+?)(&|$)/, s: "$1", follow: true },
    {
      d: "4chan.org",
      e:
        '.is_catalog .thread a[href*="/thread/"], .catalog-thread a[href*="/thread/"]',
      q: ".op .fileText a",
    ...
    { r: /de?pic\.me\/[0-9a-z]{8,}/, q: "#pic" },
    {
      ...
      s: function(m, node) {
        return /\b(film|lit)/.test(node.className) ||
          /in Flash/.test(node.title)
          ? ""
          : m.input;
      },
      q: [
    ...
    { d: "dropbox.com", r: /(.+?&size_mode)=\d+(.*)/, s: "$1=5$2" },
    {
      ...
      q: function(text) {
        return text
          .match(/https?:\/\/i\.ebayimg\.com\/[^\.]+\.JPG/i)[0]
          .replace(/~~60_\d+/, "~~60_57");
      }
    },
    {
      r: /i.ebayimg.com/,
      s: function(m, node) {
        if (qs(".zoom_trigger_mask", node.parentNode)) return "";
    ...
    },
    {
      ...
      s: function(m, node) {
        return contains(node.parentNode.className, "video") &&
          contains(m[4], "fbcdn")
          ? ""
    ...
    },
    {
      ...
      s: function(m, node) {
        if (node.id == "fbPhotoImage") return false;
        if (/gradient\.png$/.test(m.input)) return "";
        return m.input.replace("www.facebook.com", "mbasic.facebook.com");
      },
      q:
        'div + span > a:first-child:not([href*="tag_faces"]), div + span > a[href*="tag_faces"] ~ a',
      rect: "#fbProfileCover"
    },
    {
      ...
      s: function(m) {
        try {
          if (/[\.^]facebook\.com$/.test(hostname))
            return unsafeWindow.PhotoSnowlift.getInstance().stream.cache.image[
              m[1]
            ].url;
        } catch (ex) {}
        return false;
    ...
    },
    {
      ...
      s: function(m, node) {
        if (node.id == "fbPhotoImage") {
          ...
          if (a) {
            return contains(a.href, m.input.match(/[0-9]+_[0-9]+_[0-9]+/)[0])
              ? ""
              : a.href;
          }
        }
        if (m[4]) return false;
        if (contains(node.parentNode.outerHTML, "/hovercard/")) return "";
        var gp = node.parentNode.parentNode;
        if (contains(node.outerHTML, "profile") && contains(gp.href, "/photo"))
          return false;
        return (
          m[1].replace(/\/[spc][\d\.x]+/g, "").replace("/v/", "/") +
          "_n." +
          m[3]
        );
      },
    ...
    },
    {
      ...
      s: function(m) {
        return (
          "https://i.imgflip.com/" + m[2] + (m[1] == "i" ? ".jpg" : ".mp4")
        );
      }
    },
    ...
    { r: /imgtheif\.com\/image\//, q: 'a > img[src*="/pictures/"]' },
    {
      ...
      g: function(text, url, cb) {
        var mk = function(o, imgs) {
            var items = [];
            if (!o || !imgs) return items;
            for (
              var i = 0, len = imgs.length, cur;
              i < len && (cur = imgs[i]);
              i++
            ) {
          ...
          m = /(mergeConfig\('gallery',\s*|Imgur\.Album\.getInstance\()(\{[\s\S]+?\})\);/.exec(
            text
          ),
          o1 = eval(
            "(" +
              m[2]
                .replace(/analytics\s*:\s*analytics/, "analytics:null")
                .replace(/decodeURIComponent\(.+?\)/, "null") +
              ")"
          ),
          o = o1.image || o1.album,
          imgs = o.is_album ? o.album_images.images : [o];
        if (!o.num_images || o.num_images <= imgs.length) return mk(o, imgs);
        GM_xmlhttpRequest({
          method: "GET",
          url:
            "https://imgur.com/ajaxalbums/getimages/" +
            o.hash +
            "/hit.json?all=true",
          onload: function(res) {
            var imgs;
    ...
    },
    {
      ...
      s: function(m, node) {
        if (/memegen|random|register|search|signin/.test(m.input)) return "";
        if (
          /(i\.([a-z]+\.)?)?imgur\.com\/(a\/|gallery\/)?/.test(
    ...
    },
    {
      ...
      q: function(text) {
        ...
        return (
          m.video_url ||
          m.display_url.replace(/\/[sp]\d+x\d+\//, "/").replace(/\?.+/, "")
        );
      },
      rect: "div.PhotoGridMediaItem",
      c: function(text) {
        var m = JSON.parse(text).graphql.shortcode_media.edge_media_to_caption
          .edges[0];
        if (m === undefined) {
          return "(no caption)";
        }
    ...
    },
    {
      ...
      g: function(text) {
        ...
        items.title = o.name;
        for (
          var i = 0, len = o.items.length, cur;
          i < len && (cur = o.items[i]);
          i++
        ) {
          items.push({
    ...
    { r: /postima?ge?\.org\/image\/\w+/, q: ['a[href*="dl="]', "#main-image"] },
    {
      ...
      q: function(text) {
        return text.match(
          /http:\/\/[a-z0-9]+\.radikal\.ru[a-z0-9\/]+\.(jpg|gif|png)/i
        )[0];
      }
    },
    ...
    { r: /turboimagehost\.com\/p\//, q: "#imageid", xhr: true },
    {
      ...
      s: "/_(reasonably_small|normal|bigger|d+xd+)\\././g"
    },
    {
    ...
    },
    {
      ...
      s: function(m, node) {
        return /http[^"]+/.exec(
          node.style.cssText + node.getAttribute("data-img-src")
        )[0];
      },
      follow: true
    },
    ...
    },
    {
      ...
      q: function(text) {
        return text.match(
          /https?:\/\/twitter\.com\/[^\/]+\/status\/\d+\/photo\/\d+/i
        )[0];
      },
      follow: true
    },
    ...
    },
    {
      ...
      q:
        ".OldMedia img, .media img, video.animated-gif, .AdaptiveMedia-singlePhoto img, .AdaptiveMedia-halfWidthPhoto img, .AdaptiveMedia-twoThirdsWidthPhoto img, .AdaptiveMedia-threeQuartersWidthPhoto img",
      follow: function(url) {
        return !/\.mp4$/.test(url);
      }
    },
    ...
    },
    {
      ...
      q: function(text, doc) {
        var node = findNode(
          ["div.jp-jplayer", 'meta[property="og:image"]'],
          doc
        );
    ...
    },
    {
      ...
      s:
        "/\\/thumb(?=\\/)|\\/scale-to-width(-[a-z]+)?\\/[0-9]+|\\/revision\\/latest|\\/[^\\/]+$//g",
...
function onMouseMove(e) {
  ...
  if (_.zoom) {
    ...
    setStatus(
      "edge",
      _.cx < bx ||
        _.cx > _.view.width - bx ||
        _.cy < by ||
        _.cy > _.view.height - by
        ? "add"
        : "remove"
    );
  }
  ...
  switch (e.keyCode) {
    ...
    case 84:
      ...
      if (
        ...
        !_.xhr &&
        tag(_.popup) == "IMG" &&
        contains(navigator.userAgent, "Gecko/")
      )
        GM_openInTab(
          "data:text/html;," +
            encodeURIComponent(
...
function onContext(e) {
  ...
  if (
    (cfg.start == "context" || (cfg.start == "auto" && _.manual)) &&
    !_.status &&
    !_.popup
  ) {
    startPopup();
...
function onMessage(e) {
  if (
    !contains(trusted, e.origin.substr(e.origin.indexOf("//") + 2)) ||
    typeof e.data != "string" ||
    e.data.indexOf("mpiv-rule ") !== 0
  )
    return;
...
function startSinglePopup(url) {
  ...
  parsePage(url, function(iurl, cap, url) {
    ...
    if (
      _.follow === true ||
      (typeof _.follow == "function" && _.follow(iurl))
    ) {
    ...
    _.iurl = iurl;
    if (_.xhr) downloadImage(iurl, url);
    else setPopup(iurl);
  });
}
    ...
    for (
      ...
      try {
        ...
        item.desc = qC.reduce(function(prev, q) {
          var n = qs(q, node);
          if (!n) {
            [node.previousElementSibling, node.nextElementSibling].forEach(
              function(es) {
...
function nextGalleryItem(dir) {
  if (dir > 0 && (_.gIndex += dir) >= _.gItems.length) _.gIndex = 0;
  else if (dir < 0 && (_.gIndex += dir) < 0) _.gIndex = _.gItems.length - 1;
  ...
  if (tag(node) == "A") {
    ...
    a =
      tag(node.parentNode) == "A"
        ? node.parentNode
        : tag(node.parentNode.parentNode) == "A"
...
function findInfo(url, node, noHtml, skipHost) {
  for (
    var i = 0, len = hosts.length, tn = tag(node), h, m, html, urls;
    i < len && (h = hosts[i]);
    i++
  ) {
    ...
    if ("s" in h) {
      urls = (Array.isArray(h.s) ? h.s : [h.s]).map(function(s) {
        if (typeof s == "string") return decodeURIComponent(replace(s, m));
        if (typeof s == "function") return s(m, node);
        return s;
      });
      if (h.q && urls.length > 1) {
        console.log(
          "Rule discarded. Substitution arrays can't be combined with property q."
        );
        continue;
      }
    ...
    if (
      (h.follow === true ||
        (typeof h.follow == "function" && h.follow(urls[0]))) &&
      !h.q &&
      h.s
    )
    ...
    if (
      (contains(hostname, "twitter.com") &&
        !/(facebook|google|twimg|twitter)\.com\//.test(info.url)) ||
      (hostname == "github.com" && !/github/.test(info.url)) ||
      (contains(hostname, "facebook.com") && /\bimgur\.com/.test(info.url))
    )
      info.xhr = "data";
    return info;
  }
    ...
    onload: function(res) {
      try {
        ...
        var txt = res.responseText,
          ui8 = new Uint8Array(txt.length),
          type;
        for (var i = txt.length; i--; ) {
          ui8[i] = txt.charCodeAt(i);
        }
        if (
          /Content-Type:\s*(.+)/i.exec(res.responseHeaders) &&
          !contains(RegExp.$1, "text/plain")
        )
          type = RegExp.$1;
        if (!type) {
          var ext = /\.([a-z0-9]+?)($|\?|#)/i.exec(url)
...
function findFile(n, url) {
  ...
  var path =
    ...
    (/https?:\/\/[.\/a-z0-9_+%\-]+\.(jpe?g|gif|png|svg|webm|mp4)/i.exec(
      n.outerHTML
    ) &&
      RegExp.lastMatch);
  return path
    ? rel2abs(path.trim(), base ? base.getAttribute("href") : url)
    : false;
}
...
function checkProgress(start) {
  ...
  if (
    (_.large =
      _.nwidth > p.clientWidth + _.mbw || _.nheight > p.clientHeight + _.mbh)
  )
    setStatus("large");
...
function updateScales() {
  var scales = cfg.scales.length
    ...
    fit = Math.min(
      (_.view.width - _.mbw) / _.nwidth,
      (_.view.height - _.mbh) / _.nheight
    ),
    cutoff = (_.scale = Math.min(1, fit));
  _.scales = [];
  for (var i = scales.length; i--; ) {
    var val = parseFloat(scales[i]) || fit,
      opt = typeof scales[i] == "string" ? scales[i].slice(-1) : 0;
    if (opt == "!") cutoff = val;
    if (opt == "*") _.zscale = val;
...
function showFileInfo() {
  if (_.gItems) {
    ...
    var c =
      _.gItems.length > 1
        ? "[" + (_.gIndex + 1) + "/" + _.gItems.length + "] "
        : "";
    if (
      _.gIndex == 0 &&
      _.gItems.title &&
      (!item.desc || !contains(item.desc, _.gItems.title))
    )
...
function placePopup() {
  ...
  var x = null,
    y = null,
    w = Math.round(_.scale * _.nwidth),
    h = Math.round(_.scale * _.nheight),
    cx = _.cx,
    cy = _.cy,
    vw = _.view.width,
    vh = _.view.height;
...
function toggleZoom() {
  ...
  _.scale =
    _.scales[
      _.zoom
        ? _.scales.indexOf(_.zscale) > 0
          ? _.scales.indexOf(_.zscale)
          : 1
        : 0
    ];
...
function handleError(o) {
  ...
  if (
    contains(hostname, "google") &&
    contains(location.search, "tbm=isch") &&
    !_.xhr &&
    cfg.xhr
  ) {
    _.xhr = true;
...
function setPopup(src) {
  ...
  if (!src) return;
  if (
    (src.substr(0, 5) != "data:" && /\.(webm|mp4)($|\?)/.test(src)) ||
    src.substr(0, 10) == "data:video"
  ) {
    var start = Date.now(),
      bar;
...
function findScale(url, parent) {
  ...
  for (var i = imgs.length, img; i-- && (img = imgs[i]); ) {
    ...
    var s = Math.max(
      (img.naturalHeight || img.videoHeight) / img.offsetHeight,
...
function setup() {
  ...
  var close = function() {
    rm($("setup"));
    if (!contains(trusted, hostname)) off(wn, "message", onMessage);
  };
  var update = function() {
    $("delay").parentNode.style.display = $(
      "preload"
    ).parentNode.style.display = $("start-auto").selected ? "" : "none";
  };
  ...
  var getCfg = function() {
    ...
    cfg.start = $("start-context").selected
      ? "context"
      : $("start-ctrl").selected
      ? "ctrl"
      : "auto";
    cfg.zoom = $("zoom-context").selected
      ? "context"
      : $("zoom-wheel").selected
      ? "wheel"
      : $("zoom-shift").selected
  ...
  var init = function(cfg) {
    ...
    if (cfg.hosts) {
      ...
      if (lines.length > 5 || setup.search) {
        var se = $("search"),
          sf = function() {
            var inps = qsa("input", $("hosts")),
              s = se.value.toLowerCase();
            setup.search = s;
            for (var i = 0; i < inps.length; i++) {
              inps[i].style.display =
                !inps[i].value || contains(inps[i].value.toLowerCase(), s)
                  ? ""
                  : "none";
            }
          };
    ...
    $("hosts").style.maxHeight =
      parseInt($("hosts").offsetHeight + 0.8 * free) + "px";
