...
| `git`     | [`git`](https://git-scm.com/), [`fzf`](https://github.com/junegunn/fzf) (optional) | Minimal git configuration file + git aliases (`gcmsg`, `gl`, `gp`, etc). The alias `gdiff` requires `fzf`.        | `.config/git/config.local` should be created with credentials under `[user]` if needed. |
...
| `steam`                | `wget`                          | [`steam`](https://store.steampowered.com/); (make: [`git`](https://git-scm.com), `wget`, `unzip`)                                                                                | Installs the metro-for-steam skin and the UPMetroSkin patch for Steam.                                         |                                                             |
| `terminfo`             |                                 | `terminfo`                                                                                                                                                                       | Sets `TERMINFO` to be XDG compliant.                                                                           |                                                             |
| `termite`              | `dotprofile`, `gtk`, `terminfo` | [`termite`](https://github.com/thestinger/termite)                                                                                                                               | Templated configuration to use profile colors and font.                                                        |                                                             |
